...
experimentalists. We present KinBench-UQ, an artifact-backed benchmark for
...
analysis, and external validation. Second, it packages that benchmark in an
artifact-reproducible form: the result summaries, figures, and manuscript
artifacts are generated directly from versioned result files.
...
The strongest reproducibility guarantee in the current repository is therefore
at the artifact level: saved CSV, JSON, and figure outputs are versioned and
can be regenerated from the processed dataset and benchmark scripts, while the
initial raw-data acquisition still depends on public external sources.

\section{Results}

\subsection{Main Benchmark Results}

Table~\ref{tab:main-results} summarizes the current four-split comparison. All
...
three strongest models on each split by current saved results. Lower RMSE is
