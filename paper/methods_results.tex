...
Among the currently generated external runs, \texttt{deepdta\_exact} attains
the strongest performance on the BindingDB-derived evaluation set with RMSE
1.342 and mean per-target Spearman 0.361. The repository model
\texttt{dual\_tower\_uq} follows with RMSE 1.484 and mean per-target Spearman
0.342, while \texttt{graphdta\_gcn\_exact} reaches RMSE 1.538 and mean
per-target Spearman 0.138. The ridge baselines are weaker still, with RMSE
1.602 for the target-aware ridge ensemble and 1.641 for the ligand-only ridge
baseline. These numbers are worse than the in-domain Davis results, but they
are more scientifically credible for a candidate-prioritization claim.
